	...
	,m_hasArrow(hasArrow)
{
	...
	//设置动画曲线，曲率
	//动画缓和曲线类型，从0开始以(t^2）进行加速，可以看QT帮助文档
...
{
	//创建第一个画家
	QPainter painter(this);		//写this,是在当前窗口画
	...
	if (m_hasArrow)
	{
		...
		//顺时针，旋转坐标系
		//进行旋转，m_rotation就是旋转角度传进去
		p.rotate(m_rotation);

		//旋转之后，在这个位置，进行图片【绘制】
		//因为之前进行过坐标偏移，所有用0减去坐标的一半
		p.drawPixmap(0 - pixmap.width() / 2, 0 - pixmap.height() / 2, pixmap);

		/*
			画家p 是在中间位图tmpPixmap上画的
			接下来，要画到第一个画家上去
			设备时当前窗口，就是this上面
		*/

		//原来的画家，绘制位图
		//画的图片就是p画家在tmpPixmap上面画的图
